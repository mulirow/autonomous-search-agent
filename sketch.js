...
function draw() {
  ...
  if (
    p5.Vector.dist(
      agent.pos,
      createVector(
        grid.endPoint[0] * grid.cellSize + grid.cellSize / 2,
        grid.endPoint[1] * grid.cellSize + grid.cellSize / 2
      )
    ) == 0
  ) {
    ...
    do {
      ...
      grid.terrainColors[grid.terrain[endRow][endCol]] == grid.terrainColors[0]
    );
